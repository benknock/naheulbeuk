...
do
    ...
    do
        ...
        comp='{"compendium":"'
        comp2='", '
        echo "$comp$list$comp2$line2**********" | sed 's/\\"/|"/g' | sed 's/"} //g' | sed 's/} //g' | sed 's/]{/]/g' | sed 's/{"/{||||/g' | sed 's/"}/||||}/g' | sed 's/,"/,||||/g' | sed 's/",/||||,/g' | sed 's/:"/:||||/g' | sed 's/":/||||:/g' | sed 's/"//g' | sed 's/||||/"/g' | sed 's/", /, /g' >>../../all.txt
    done
    cd ../..
done
allObj=$(cat all.txt)
...
do
    ...
    do
        ...
        comp='{"compendium":"'
        comp2='", '
        echo "$comp$list$comp2$line2**********"| sed 's/\\"/|"/g'>>../../all.txt
    done
    cd ../..
done
allObj=$(cat all.txt)
